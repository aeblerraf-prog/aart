...
def mp_worker(tsnap, supergrid0, mask0, sign0, rs0, phi0, t0, supergrid1, mask1, sign1, rs1, phi1, t1, supergrid2, mask2, sign2, rs2, phi2, t2, xtend, interpolated3_R, thetao, isco, i_case, spin_case, N0, N1, N2):
    # This function needs arguments because it's pickled and sent to workers
    # It cannot rely on global variables if not defined in the worker's scope
    # However, since this is multiprocess, global variables might be copied or not depending on fork.
    # We will pass what we need.
    # Wait, the original code relied on globals. In 'fork', globals are copied.
    # But for cleaner code, we might want to be careful.
    # For now, let's keep the structure as close to original but inside the check.

    # We need to reconstruct the args if we change signature.
    # Original function:
    # def mp_worker(tsnap):
...
# We define the worker globally so it can be pickled
# But we need to make sure the globals it uses are available.
# In "fork" context, they are inherited.
# So we can keep mp_worker defined globally, but we must ensure the globals are populated before pool starts.

# BUT if we move the main logic into if __name__ == "__main__", then when imported, the globals are NOT populated.
# When running as script, they are populated inside the block.
# Does 'fork' copy variables defined inside 'if __name__ == "__main__"'?
# Yes, if the pool is created inside that block.

interpolated3_R = None # Placeholder

def mp_worker_wrapper(args):
    tsnap = args
    # We assume globals are set in the worker process via fork copy-on-write
    i_bghts0 = obsint.slow_light(supergrid0,mask0,sign0,spin_case,isco,rs0,phi0,np.mod(t0+tsnap,xtend), interpolated3_R,thetao)
...
if __name__ == "__main__":
    ...
    I0s,I1s,I2s = zip(*p.map(mp_worker_wrapper, np.linspace(i_tM+i_frame,f_tM,snapshots)))
    p.close()
